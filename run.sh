...
docker run -d --restart unless-stopped --name "${APP_CONTAINER_NAME}" --memory=1024m -e POSTGRES_HOST="${DB_CONTAINER_NAME}" -e POSTGRES_PORT="${POSTGRES_PORT}" -e POSTGRES_USER="${POSTGRES_USER}" -e POSTGRES_PASSWORD="${POSTGRES_PASSWORD}" -e POSTGRES_DB="${POSTGRES_DB}" -p "${APP_PORT}":80 "${APP_CONTAINER_NAME}" # запускаем новый контейнер приложения, добавляем ограничения по памяти - 1024 мб, пробрасываем переменные окружения для подключения к базе данных, атрибут d означает, что контейнер запускается в фоновом режиме(detached)
